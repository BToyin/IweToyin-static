...
public class ApiController {
    ...
    @PutMapping("/blog/posts/{id}/edit")
    public ResponseEntity editBlogPost(@PathVariable int id, @RequestBody BlogPost blogPost) {
        Optional<BlogPost> existingBlogPost = Optional.ofNullable(blogPostService.getBlogPostById(id));
        if(existingBlogPost.isPresent()) {
            BlogPost editedBlogPost = existingBlogPost.get();
            editedBlogPost.setTitle(blogPost.getTitle());
            editedBlogPost.setRawContent(blogPost.getRawContent());
            editedBlogPost.setAuthor(blogPost.getAuthor());
            blogPostService.createNewBlogPost(editedBlogPost);
            return new ResponseEntity(HttpStatus.OK);
        } else {
            return new ResponseEntity(HttpStatus.NOT_FOUND);
        }
